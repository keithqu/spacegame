...
export class GalaxyGenerator {
  ...
  generateGalaxy(): Galaxy {
    ...
    const avgConnections = systems.reduce((sum, s) => sum + s.connections.length, 0) / systems.length;
    ...
    console.log(`✅ Generated galaxy: ${systems.length} systems, ${anomalies.length} anomalies, ${warpLanes.length} warp lanes`);
    console.log(`📊 Connectivity: ${avgConnections.toFixed(1)} avg connections, ${maxDistance.toFixed(1)} max distance, ${avgDistance.toFixed(1)} avg distance`);
    ...
    console.log(`📏 Distance distribution: ${shortLanes} short (≤4 LY), ${mediumLanes} medium (4-6 LY), ${longLanes} long (>6 LY)`);
    return galaxy;
  }
  ...
  private generateWarpLanes(systems: StarSystem[]): WarpLane[] {
    ...
    systems.forEach(system => {
      ...
      for (const candidate of candidates) {
        ...
        const probability = Math.exp(-normalizedDistance * this.config.connectivity.distanceDecayFactor);
  ...
  private ensureMinimumConnectivity(
    ...
    const isolatedSystems = systems.filter(system => 
      connections.get(system.id)!.size === 0
    );
    ...
    for (const system of isolatedSystems) {
      ...
      if (candidates.length > 0) {
        ...
        if (nearest.distance <= 15) {
          ...
          console.log(`🔗 Connected isolated system ${system.name} to ${nearest.system.name} (${nearest.distance.toFixed(1)} LY)`);
        }
      }
    }
  }
  ...
  private isPositionTooClose(
    ...
    return existingObjects.some(obj => 
      this.calculateDistance(position, obj) < minDistance
    );
  }
  ...
  private isPositionTooCloseToSystems(
    ...
    return existingSystems.some(system => 
      this.calculateDistance(position, system) < minDistance
    );
  }
