...
export function testGalaxyGeneration(): void {
  ...
  for (const system of galaxy.systems) {
    for (const anomaly of galaxy.anomalies) {
      const distance = Math.sqrt(
        Math.pow(system.x - anomaly.x, 2) +
        Math.pow(system.y - anomaly.y, 2)
      );
  ...
  console.log(`✅ Min anomaly-anomaly distance: ${minAnomalyDistance.toFixed(2)} LY (should be ≥ 2.0)`);
  console.log(`✅ Min system-anomaly distance: ${minSystemAnomalyDistance.toFixed(2)} LY (should be ≥ 3.0)`);
  ...
  const anomalyConnections = galaxy.warpLanes.filter(w => 
    w.from.startsWith('anomaly-') || w.to.startsWith('anomaly-')
  );
